...
#define MAX_VAL_BITS(a) ((1<<(bits)) - 1)
    ...
    protected:
        inline static void internal_porter_duff(float Fa, float Fb, const color_f_t & b, const color_f_t & s, color_f_t &output) {
            float as = s.a;
            float ab = b.a;
...
// (256-1)(256-1)= 2^16 - 2^8 - 2^8 + 1 = 2^16 - 2^9 + 1
            // unmultiplied-alpha result

            // optimization for opaque backdrops
