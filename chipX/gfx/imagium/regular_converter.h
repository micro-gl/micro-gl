    ...
    public:
        ...
        byte_array write(byte_array * data, const options & options) const override {
            ...
            if(options.pack_channels)
                array=factory_PackedArray::getArray(pixels_count, r_bits, g_bits, b_bits, a_bits);
            else
                array=factory_UnpackedArray::getArray(pixels_count, r_bits, g_bits, b_bits, a_bits);
